...
public class DriverResultRepository {
	...
	public List<DriverResult> getResultByYear(int year) {
		...
		try {
			...
			Connection conn = db_conexion.Conexionbd("bd_pruebas", "postgres", "12345", "postgresql");
	...
	public static void main(String[] args) {
		...
		for (DriverResult rs : ls_d) {
			System.out.printf("%-20s %-10d %-15d  %-10d%n", rs.getDriverName(),rs.getWins(),rs.getTotalPoints(),rs.getRank());
		}
	}
}
